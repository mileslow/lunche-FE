...
import { CurrentLocation, createLocationObject } from 'services/geoLocation'
// types
...
const ChangeAddressModal: FC<StackScreenProps<RootNavigationStackParamsList, Routes.ChangeAddressModal>> = ({
  ...
  const handleLocationPress = useCallback(
    (item: CurrentLocation) => {
      Keyboard.dismiss()
      const filteredLocations = reject(recent, { address: item.address })
      setRecentSearch(
  ...
  const keyExtractor = useCallback((item: CurrentLocation, index: number) => `${item.id}-${index}`, [])
  ...
  const sections = useMemo(() => {
    const data = [
      { key: LocationSectionsKeys.SearchLocations, title: '', data: searchResult },
      {
        ...
        data: map(savedLocations, (i) => ({ address: i.address, lat: i.latitude, lng: i.longitude })),
      },
